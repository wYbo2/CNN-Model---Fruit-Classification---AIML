...
if not os.path.exists(MODEL_DIR):
    print(f"Error: '{MODEL_DIR}' folder missing.")
    exit()
...
train_ds = tf.keras.utils.image_dataset_from_directory(
    f"{DATA_DIR}/train", image_size=IMG_SIZE, batch_size=BATCH_SIZE, shuffle=False
)
val_ds = tf.keras.utils.image_dataset_from_directory(
    f"{DATA_DIR}/val", image_size=IMG_SIZE, batch_size=BATCH_SIZE, shuffle=False
)
...
h = 0.35
ax2.barh(y_pos - h/2, df['train_acc'], h, label='Training', color='lightgray')
ax2.barh(y_pos + h/2, df['val_acc'], h, label='Validation', color='dodgerblue')
